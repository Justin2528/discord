...
exports.run = async (client, message, args, tools) => {
  ...
  if (!message.member.hasPermission('MANAGE_GUILD') && message.author.id !== '357555941215961099') return message.channels.send('Sorry, you don\'t have permission to create poll!');
  ...
  const embed = new Discord.MessageEmbed()
    ...
    const collector = pollTitle.createReactionCollector(filter, { time: 60000 });
      collector.on('collect', r => console.log(`Collected ${r.emoji.name}`));
      collector.on('end', collected => console.log(`Collected ${collected.size} items`));
  
    const filter1 = (reaction) => reaction.emoji.name === '👎';
    const collector1 = pollTitle.createReactionCollector(filter1, { time: 60000 });
